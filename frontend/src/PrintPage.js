...
/**
 * PrintPage
 * Displays the upcoming calendar
 * Retrieves tasks from localStorage('activeTasks')
 * Formatting in a way such as *Month Day, Year*
 * Then creates a .txt file with tasks outlined with dates then tasks
 */
...
export default function PrintPage() {
  ...
  useEffect(() => {
    try {
      ...
      setActiveTasks(Array.isArray(saved) ? saved : []);
    } catch (e) {
      setActiveTasks([]);
    }
  }, []);
  ...
  const downloadCalendar = () => {
    if (!groupedByStartDate.length) return;
    const lines = [];
    for (const [dateKey, tasks] of groupedByStartDate) {
  ...
  return (
    <div>
      <h1 className="title">Upcoming Calendar</h1>
